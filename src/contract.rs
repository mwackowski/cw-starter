...
fn execute_create_poll(
    ...
    let poll = Poll {
        creator: info.sender,
        // changed from question: question to work with attribute
    ...
    .add_attribute("question", question.to_string()))
    // Ok(Response::new())
}
...
fn execute_vote(
    ...
    match poll {
        Some(mut poll) => { // The poll exists
            ...
            .add_attribute("ballot", &poll.options[position].0))
        },
...
mod tests {
    ...
    fn test_execute_vote_valid() {
        ...
        assert_eq!(
            _res.attributes,
            vec![attr("action", "vote"), attr("ballot", "Juno".to_string())]
        );
        // Change the vote
        let msg = ExecuteMsg::Vote {
            poll_id: "123123".to_string(),
        ...
        assert_eq!(
            _res.attributes,
            vec![attr("action", "vote"), attr("ballot", "Osmosis".to_string())]
        );
    }
